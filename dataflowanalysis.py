...
def detect_connection(x):
    ...
    local_formatted_str = []
    connection_methods = [["Ljava/net/Socket", "<init>"],
                          ["Ljava/net/InetSocketAddress", "Ljava/net/InetSocketAddress"]]
    for i in range(len(connection_methods)):
        structural_analysis_results = x.tainted_packages.search_methods(connection_methods[i][0],
                                                                        connection_methods[i][1], ".")
        ...
        for result in xrange(len(structural_analysis_results)):
            ...
            # print(registers)
            if len(registers) >= 2:
                remote_address = get_register_value(1,
                                                    registers)  # 1 is the index of the PARAMETER called in the method
...
def detect_content(x):
    ...
    formatted_str = []
    content_apis = [["Landroid/content/ContentResolver", "query"], ["Landroid/content/ContentResolver", "insert"],
